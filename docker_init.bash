...
#
# Create the hermetic `hermes` CLI wrapper so the command is always on PATH
# without requiring the venv to be activated.  This is essential because:
#   1. Cron jobs started by the Hermes scheduler invoke `hermes content …`,
#      `hermes cron run …`, etc., and they run outside the venv.
#   2. Terminal sessions inside the WebUI (xterm.js) also lack venv activation.
#   3. After every deploy the wrapper must be re-created automatically, since
#      /usr/local/bin is ephemeral inside the container.
#
echo ""; echo "== Creating hermetic 'hermes' CLI wrapper in /usr/local/bin/"
_hermes_venv_bin="/app/venv/bin"
_hermes_target="${_hermes_venv_bin}/hermes"
if [ -f "$_hermes_target" ]; then
  cat > /usr/local/bin/hermes << HERMESWRAP
#!/bin/bash
# Auto-generated by docker_init.bash — do not edit.
# Ensures the 'hermes' CLI is always on PATH with the correct PYTHONPATH.
export PYTHONPATH="/opt/hermes${PYTHONPATH:+:\$PYTHONPATH}"
exec "${_hermes_target}" "\$@"
HERMESWRAP
  chmod 755 /usr/local/bin/hermes || error_exit "Failed to chmod /usr/local/bin/hermes"
  echo "-- Created /usr/local/bin/hermes -> ${_hermes_target}"
  # Smoke test: verify the wrapper works
  /usr/local/bin/hermes --help >/dev/null 2>&1 \
    && echo "-- Wrapper smoke test: PASS" \
    || echo "!! WARNING: /usr/local/bin/hermes smoke test failed (installation may be incomplete)"
else
  echo "!! WARNING: ${_hermes_target} not found — hermes CLI wrapper not created."
  echo "!!   hermes-agent[mcp,honcho] may not have been installed correctly."
fi
